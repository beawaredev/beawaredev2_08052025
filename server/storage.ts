...
export interface IStorage {
  ...
  updateScamReportVerification(
    id: number,
    isVerified: boolean,
    verifiedBy: number,
  ): Promise<ScamReport | undefined>;
  toggleScamReportPublished(
    id: number,
    isPublished: boolean,
    publishedBy: number,
  ): Promise<ScamReport | undefined>;
  ...
  getConsolidatedScamByIdentifier(
    identifier: string,
  ): Promise<ConsolidatedScam | undefined>;
  ...
  getSecurityChecklistItem(
    id: number,
  ): Promise<SecurityChecklistItem | undefined>;
  createSecurityChecklistItem(
    item: InsertSecurityChecklistItem,
  ): Promise<SecurityChecklistItem>;
  updateSecurityChecklistItem(
    itemId: number,
  ...
  updateUserSecurityProgress(
    userId: number,
    checklistItemId: number,
    isCompleted: boolean,
    notes?: string,
  ): Promise<UserSecurityProgress>;
  getUserSecurityProgressForItem(
    userId: number,
    checklistItemId: number,
...
export class MemStorage implements IStorage {
  ...
  private scamReportConsolidations: Map<number, ScamReportConsolidation> =
    new Map();
  ...
  private securityChecklistItems: Map<number, SecurityChecklistItem> =
    new Map();
  ...
  async getUserByBeawareUsername(
    beawareUsername: string,
  ): Promise<User | undefined> {
    return Array.from(this.users.values()).find(
      (user) => user.beawareUsername === beawareUsername,
    );
  }
  ...
  async createScamReport(insertReport: InsertScamReport): Promise<ScamReport> {
    ...
    const incidentDate =
      typeof insertReport.incidentDate === "string"
  ...
  private async createConsolidatedScam(
    insertScam: InsertConsolidatedScam,
  ): Promise<ConsolidatedScam> {
  ...
  async updateScamReportVerification(
    ...
    if (isVerified) {
      const consolidations = Array.from(
        this.scamReportConsolidations.values(),
      ).filter((c) => c.scamReportId === id);

      for (const consolidation of consolidations) {
        const consolidatedScam = await this.getConsolidatedScam(
  ...
  async getScamComments(scamReportId: number): Promise<ScamComment[]> {
    return Array.from(this.scamComments.values()).filter(
      (comment) => comment.scamReportId === scamReportId,
    );
  }
  ...
  async getConsolidatedScamByIdentifier(
    identifier: string,
  ): Promise<ConsolidatedScam | undefined> {
    return Array.from(this.consolidatedScams.values()).find(
      (scam) => scam.identifier.toLowerCase() === identifier.toLowerCase(),
    );
  }
  ...
  async searchConsolidatedScams(query: string): Promise<ConsolidatedScam[]> {
    ...
    return Array.from(this.consolidatedScams.values()).filter((scam) =>
      scam.identifier.toLowerCase().includes(lowerQuery),
    );
  }

  async createLawyerProfile(
    profile: InsertLawyerProfile,
  ...
  async getLawyerProfileByUserId(
    userId: number,
  ): Promise<LawyerProfile | undefined> {
    return Array.from(this.lawyerProfiles.values()).find(
  ...
  async createLawyerRequest(
    request: InsertLawyerRequest,
  ): Promise<LawyerRequest> {
  ...
  async getLawyerRequestsByUser(userId: number): Promise<LawyerRequest[]> {
    return Array.from(this.lawyerRequests.values()).filter(
  ...
  async getFeaturedScamVideos(): Promise<ScamVideo[]> {
    return Array.from(this.scamVideos.values()).filter(
  ...
  async getSecurityChecklistItem(
    id: number,
  ): Promise<SecurityChecklistItem | undefined> {
  ...
  async getUserSecurityProgress(
    userId: number,
  ): Promise<UserSecurityProgress[]> {
    return Array.from(this.userSecurityProgress.values()).filter(
  ...
  async getUserSecurityProgressForItem(
    ...
    return Array.from(this.userSecurityProgress.values()).find(
      (progress) =>
        progress.userId === userId &&
        progress.checklistItemId === checklistItemId,
    );
  }
  ...
  async updateUserSecurityProgress(
    ...
    const existing = await this.getUserSecurityProgressForItem(
      userId,
      checklistItemId,
    );
    const now = new Date().toISOString();
  ...
  private async seedSecurityChecklistItems(): Promise<void> {
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Place security freezes on your credit reports with all three major credit bureaus",
      category: "identity_protection",
      priority: "high",
      recommendationText:
        "Contact Experian, Equifax, and TransUnion to freeze your credit reports. This prevents new accounts from being opened in your name without your explicit permission.",
      helpUrl:
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Set up regular monitoring of your credit reports for suspicious activity",
      category: "identity_protection",
      priority: "high",
      recommendationText:
        "Use a free service like Credit Karma or get your free annual credit reports from annualcreditreport.com to monitor for unauthorized accounts or inquiries.",
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Install and start using a reputable password manager for all your accounts",
      category: "password_security",
      priority: "high",
      recommendationText:
        "Choose a password manager like Bitwarden, 1Password, or LastPass. This allows you to use unique, strong passwords for every account without having to remember them all.",
    ...
    await this.createSecurityChecklistItem({
      ...
      priority: "high",
      recommendationText:
        "Never reuse passwords across accounts. If one account is compromised, unique passwords prevent attackers from accessing your other accounts.",
    ...
    await this.createSecurityChecklistItem({
      ...
      priority: "high",
      recommendationText:
        "Enable 2FA using an authenticator app like Google Authenticator or Authy. This adds an extra layer of security even if your password is compromised.",
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Audit and remove unnecessary permissions for apps and browser extensions",
      category: "account_security",
      priority: "medium",
      recommendationText:
        "Go through your social media accounts, Google/Apple accounts, and browser extensions. Remove apps you don't use and limit permissions for those you keep.",
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Set up login and transaction alerts for banking and email accounts",
      category: "account_security",
      priority: "medium",
      recommendationText:
        "Enable notifications for logins, password changes, and transactions. This helps you quickly detect unauthorized access to your accounts.",
      estimatedTimeMinutes: 15,
      sortOrder: 7,
    });
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Enable fingerprint, face recognition, or other biometric security on your devices",
      category: "device_security",
      priority: "medium",
      recommendationText:
        "Set up fingerprint or face unlock on your phone, tablet, and computer when available. This is more secure than PINs and harder for others to bypass.",
    ...
    await this.createSecurityChecklistItem({
      ...
      priority: "high",
      recommendationText:
        "Turn on automatic updates for your operating system, browser, and apps. Security patches often fix vulnerabilities that could be exploited by scammers.",
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Use WPA3 security and a strong password for your home Wi-Fi",
      category: "network_security",
      priority: "medium",
      recommendationText:
        "Change the default router password, use WPA3 (or WPA2 if WPA3 isn't available), and set a strong Wi-Fi password. Consider hiding your network name (SSID).",
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Install a reputable VPN service for secure browsing on public networks",
      category: "network_security",
      priority: "medium",
      recommendationText:
        "When using public Wi-Fi, use a VPN to encrypt your internet traffic. Choose a reputable service like NordVPN, ExpressVPN, or Surfshark.",
      estimatedTimeMinutes: 15,
      sortOrder: 11,
    });
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Set up a monthly routine to review all financial statements for unauthorized transactions",
      category: "financial_security",
      priority: "high",
      recommendationText:
        "Check your bank and credit card statements monthly for any transactions you don't recognize. Report suspicious activity immediately to your financial institution.",
      estimatedTimeMinutes: 20,
      sortOrder: 12,
    });
    ...
    await this.createSecurityChecklistItem({
      ...
      description:
        "Enable real-time alerts for all banking and credit card transactions",
      category: "financial_security",
      priority: "high",
      recommendationText:
        "Most banks and credit card companies offer text or email alerts for transactions. Set these up to be notified immediately of any account activity.",
      estimatedTimeMinutes: 10,
      sortOrder: 13,
    });
  }
...
export const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          "Invalid file type. Only PDF, JPG, PNG, GIF, and DOC/DOCX files are allowed.",
        ) as any,
        false,
      );
    }
