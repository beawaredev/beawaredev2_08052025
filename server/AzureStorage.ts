...
export class AzureStorage implements IStorage {
  ...
  async getFeaturedScamVideos(): Promise<ScamVideo[]> {
    ...
    try {
      const result = await pool
        .request()
        .query(
          "SELECT * FROM scam_videos WHERE is_featured = 1 ORDER BY created_at DESC",
        );
  ...
  // ---- ADD THIS INSIDE export class AzureStorage { ... } ----
  async addScamVideo(video: InsertScamVideo): Promise<ScamVideo | undefined> {
    try {
      ...
      const consolidated_scam_id =
        video.consolidated_scam_id ?? v.consolidatedScamId ?? null;
      ...
      request
        ...
        .input("video_url", sql.NVarChar(2048), video_url)
        .input("thumbnail_url", sql.NVarChar(2048), thumbnail_url)
        .input("scam_type", sql.NVarChar(50), scam_type)
        .input("consolidated_scam_id", sql.Int, consolidated_scam_id)
        .input("is_featured", sql.Bit, is_featured)
        .input("view_count", sql.Int, view_count)
        .input("duration", sql.Int, duration)
        .input("created_by", sql.Int, created_by);

      const result = await request.query(`
        INSERT INTO dbo.scam_videos
          (title, description, video_url, thumbnail_url, scam_type, consolidated_scam_id,
           is_featured, view_count, duration, created_by, created_at, updated_at)
        OUTPUT INSERTED.*
        VALUES
          (@title, @description, @video_url, @thumbnail_url, @scam_type, @consolidated_scam_id,
           @is_featured, @view_count, @duration, @created_by, GETDATE(), GETDATE());
      `);
      ...
      return {
        ...
        consolidated_scam_id: r.consolidated_scam_id,
  ...
  async updateScamVideo(
    ...
    patch: {
      title?: string;
      description?: string;
      video_url?: string; // storage
      youtubeUrl?: string; // UI alias
      videoUrl?: string; // UI alias
      thumbnail_url?: string; // storage
      thumbnailUrl?: string; // UI alias
      scam_type?: string; // storage
      scamType?: string; // UI alias
      consolidated_scam_id?: number; // storage
      consolidatedScamId?: number; // UI alias
      is_featured?: boolean; // storage
      featured?: boolean; // UI alias
      view_count?: number; // storage
      viewCount?: number; // UI alias
      duration?: number;
    },
    ...
    try {
      ...
      // Map aliases → snake_case, use null to mean "no change" (COALESCE will keep existing)
      ...
      const video_url =
        ...
        (typeof patch.youtubeUrl === "string" && patch.youtubeUrl) ||
        (typeof patch.videoUrl === "string" && patch.videoUrl) ||
        null;

      const thumbnail_url =
        (typeof patch.thumbnail_url === "string" && patch.thumbnail_url) ||
        (typeof patch.thumbnailUrl === "string" && patch.thumbnailUrl) ||
        null;

      const scam_type =
        (typeof patch.scam_type === "string" && patch.scam_type) ||
        (typeof patch.scamType === "string" && patch.scamType) ||
        null;
      ...
      const duration =
        typeof patch.duration === "number" ? patch.duration : null;
      ...
      const result = await request.query(`
        UPDATE dbo.scam_videos
        SET
          title                 = COALESCE(@title, title),
          description           = COALESCE(@description, description),
          video_url             = COALESCE(@video_url, video_url),
          thumbnail_url         = COALESCE(@thumbnail_url, thumbnail_url),
          scam_type             = COALESCE(@scam_type, scam_type),
          consolidated_scam_id  = COALESCE(@consolidated_scam_id, consolidated_scam_id),
  ...
  async deleteScamVideo(id: number): Promise<boolean> {
    try {
      ...
      const result = await request.query(`
        DELETE FROM dbo.scam_videos
        WHERE id = ${id};
      `);

      // rowsAffected[0] is the count for the DELETE statement
  ...
  async createPasswordReset(
    ...
    try {
      ...
      await pool
        ...
        .query(
          "UPDATE password_resets SET used = 1 WHERE user_id = @userId AND used = 0",
        );
