/* ---------- Bind-safe request helper that does NOT depend on getPool symbol ---------- */
let __poolPromise: Promise<sql.ConnectionPool> | null = null;

/** Ensure we have a connected pool without assuming a specific helper exists. */
async function ensurePool(): Promise<sql.ConnectionPool> {
  // Reuse existing module-level pool if present and connected
  try {
    if (
      typeof pool !== "undefined" &&
      (pool as any) &&
      (pool as any).connected
    ) {
      return pool as sql.ConnectionPool;
    }
  } catch {}

  // If a function named getPool exists (from this module), use it
  try {
    // @ts-ignore
  ...
  // Otherwise connect fresh (idempotent via __poolPromise)
  if (__poolPromise) return __poolPromise;
  __poolPromise = (async () => {
    const cfg = resolveSqlConfig();
    const p = await sql.connect(cfg as any);
    try {
      (p as any).on("error", (err: any) =>
        console.error("[SQL POOL ERROR]", err),
      );
    } catch {}
    // cache on module-level pool variable if it exists
    try {
      (pool as any) = p;
    } catch {}
    return p;
  })();
  return __poolPromise;
}

/** Always returns a ready sql.Request, independent of 'this' binding. */
async function sqlRequest() {
  const p = await ensurePool();
  return p.request();
}
import * as __db from "./db.js";
import sql from "mssql";
// Resolve pool from either named export or default export for compatibility
const pool: sql.ConnectionPool = (__db as any).pool || (__db as any).default;
if (!pool) {
  throw new Error(
    "[AzureStorage] Database pool not exported from ./db.js. Export `pool` or default ConnectionPool.",
  );
}
...
export class AzureStorage implements IStorage {
  private async getRequest() {
    ...
    try {
      if (
        (pool as any).connected === false &&
        typeof (pool as any).connect === "function"
      ) {
  ...
  private async ensureConnection(): Promise<void> {
    if (!pool) {
      throw new Error("Database pool is undefined. Check ./db.js export.");
    }
    try {
      if (pool.connected) {
        return;
  ...
  private async getPool(): Promise<sql.ConnectionPool> {
    // Use existing pool imported from ./db.js; ensure it's connected
    if (
      (pool as any) &&
      (pool as any).connected === false &&
      typeof (pool as any).connect === "function"
  ...
  async getUnverifiedScamReports(
    page: number = 1,
    limit: number = 50,
  ): Promise<ScamReport[]> {
    try {
  ...
  async getUnpublishedScamReports(
    page: number = 1,
    limit: number = 50,
  ): Promise<ScamReport[]> {
    try {
  ...
  async getAllSecurityChecklistItems(): Promise<SecurityChecklistItem[]> {
    try {
      ...
      console.error(
        "Error getting security checklist items from database:",
        error,
      );
      ...
      return [
        {
          ...
          description:
            "Place security freezes on your credit reports at all three major credit bureaus",
          category: "financial_security",
          priority: "high",
          recommendationText:
            "Contact Equifax, Experian, and TransUnion to place free security freezes on your credit reports. This prevents new accounts from being opened in your name.",
          helpUrl:
        ...
        },
        {
          ...
          priority: "high",
          recommendationText:
            "Check your bank and credit card statements weekly for suspicious activity. Set up account alerts for transactions over a certain amount.",
          helpUrl:
        ...
        },
        {
          ...
          priority: "medium",
          recommendationText:
            "Enable automatic updates for your operating system, web browsers, and important software to protect against security vulnerabilities.",
        ...
        },
        {
          ...
          priority: "medium",
          recommendationText:
            "Use WPA3 encryption, change default router passwords, and regularly update router firmware.",
          helpUrl:
        ...
        },
        {
          ...
          priority: "medium",
          recommendationText:
            "Avoid accessing sensitive information on public Wi-Fi. Use a VPN when necessary and ensure websites use HTTPS.",
        ...
        },
        {
          ...
          priority: "medium",
          recommendationText:
            "Regularly review and update privacy settings on social media platforms to limit who can see your personal information.",
          helpUrl:
            "https://www.ftc.gov/tips-advice/business-center/privacy-and-security/social-media",
          toolLaunchUrl: null,
          youtubeVideoUrl: null,
          estimatedTimeMinutes: 30,
          sortOrder: 8,
        ...
        },
        {
          ...
          priority: "medium",
          recommendationText:
            "Be suspicious of unexpected emails, verify sender identity, and never click links or download attachments from unknown sources.",
        ...
        },
        {
          ...
          priority: "high",
          recommendationText:
            "Regularly backup important files to multiple locations including cloud storage and external drives using the 3-2-1 backup rule.",
        ...
        },
        {
          ...
          priority: "medium",
          recommendationText:
            "Install reputable antivirus software and keep it updated. Enable real-time protection and regular system scans.",
        ...
        },
        {
          ...
          priority: "high",
          recommendationText:
            "Check your credit reports from all three bureaus annually at annualcreditreport.com and dispute any errors immediately.",
        ...
        },
        {
          ...
          priority: "low",
          recommendationText:
            "Use encrypted messaging apps like Signal or WhatsApp for sensitive communications instead of regular SMS.",
  ...
  async createSecurityChecklistItem(
    ...
    try {
      ...
      // Build query based on available columns
      let insertColumns =
        "title, description, category, priority, recommendation_text, help_url, estimated_time_minutes, sort_order, is_active, created_at, updated_at";
      let insertValues =
        "@title, @description, @category, @priority, @recommendationText, @helpUrl, @estimatedTimeMinutes, @sortOrder, @isActive, GETDATE(), GETDATE()";
  ...
  async getUserSecurityProgress(
    ...
    try {
      ...
      console.error(
        "Error getting user security progress from database:",
        error,
      );
      return [];
    }
  ...
  async updateUserSecurityProgress(
    ...
    try {
      ...
      console.error(
        "Error updating user security progress in database:",
        error,
      );
      throw error;
    }
  ...
  async addScamVideo(video: InsertScamVideo) {
    try {
      ...
      return {
        ...
        youtubeUrl: insertedRow.video_url,
        ...
        updatedAt: insertedRow.updated_at
          ? new Date(insertedRow.updated_at).toISOString()
          : undefined,
      };
    } catch (error) {
  ...
  async getScamVideo(id: number): Promise<ScamVideo | undefined> {
    const req = await sqlRequest();
    req.input("id", sql.Int, id);
    const result = await req.query(`
      SELECT id, title, description, video_url, scam_type, is_featured, thumbnail_url, created_by, created_at, updated_at
      FROM scam_videos WHERE id = @id
    `);
    ...
    return {
      ...
      youtubeUrl: r.video_url,
      scamType: r.scam_type ?? "business",
  ...
  async updateUser(userId: number, updateData: any): Promise<User | undefined> {
    try {
      ...
      if (updateData.displayName !== undefined) {
        updateFields.push(
          `display_name = '${(updateData.displayName || "").replace(/'/g, "''")}'`,
        );
      }
      if (updateData.beawareUsername !== undefined) {
        updateFields.push(
          `beaware_username = '${(updateData.beawareUsername || "").replace(/'/g, "''")}'`,
        );
      }
      if (updateData.role !== undefined) {
        updateFields.push(
          `role = '${(updateData.role || "user").replace(/'/g, "''")}'`,
        );
      }
    ...
    try {
      ...
      try {
        ...
        if (constraintResult.recordset.length > 0) {
          ...
          console.log(
            "Successfully removed unique constraint on beaware_username",
          );
          return;
        }
      } catch (error1) {
      ...
      try {
        await request.query(
          `ALTER TABLE users DROP CONSTRAINT UQ__users__71364CAB401C1DD1`,
        );
        console.log("Successfully removed constraint using known name");
      } catch (error3) {
        console.log("Method 3 failed:", error3);
        console.log(
          "No unique constraint found on beaware_username or already removed",
        );
      }
    } catch (error) {
...
/** Back-compat: routes.ts imports these named functions */
export async function listScamVideos() {
  return azureStorage.getAllScamVideos();
}

export async function listFeaturedScamVideos() {
  return azureStorage.getFeaturedScamVideos();
}
