...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          "Invalid file type. Only PDF, DOC, DOCX, JPG, PNG, GIF, and TXT files are allowed.",
        ),
      );
    }
  },
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const requireAdmin = async (
    ...
    if (headerUserId && headerEmail && headerRole === "admin") {
      console.log("✅ Admin access granted via headers:", {
        id: headerUserId,
        email: headerEmail,
        role: headerRole,
      });
    ...
    if (headerEmail || headerUserId) {
      try {
        ...
        if (verifiedUser && verifiedUser.role === "admin") {
          console.log("✅ Admin access granted via database verification:", {
            id: verifiedUser.id,
  ...
  apiRouter.get("/debug/auth", async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      debug: {
        ...
        recommendations: [
          headers.userId && headers.userEmail && headers.userRole
            ? "✅ Auth headers present"
            : "❌ Auth headers missing - check localStorage",
    ...
    try {
      ...
      if (!fileExists(filePath)) {
        ...
        const matchingFile = files.find((file) => {
          ...
          return (
            fileWithoutTimestamp === decodedFilename ||
    ...
    try {
      ...
      try {
        ...
        if (userData.beawareUsername) {
          const existingUsernameUser = await storage.getUserByBeawareUsername(
            userData.beawareUsername,
          );
          if (existingUsernameUser) {
            return res.status(400).json({
              message:
                "This BeAware username is already taken. Please choose a different one.",
            });
          }
        }
  ...
  apiRouter.post(
    "/auth/remove-username-constraint",
    async (req: Request, res: Response) => {
      try {
        ...
        res.json({
          success: true,
          message:
            "Removed unique constraint on beaware_username to allow multiple null values",
        });
      } catch (error) {
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const existingUser =
          await storage.getUserByBeawareUsername(newUsername);
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        // Check if username is already taken
        const existingUser =
          await storage.getUserByBeawareUsername(beawareUsername);
        if (existingUser) {
          return res.status(400).json({ message: "Username is already taken" });
        }
  ...
  apiRouter.post("/auth/google-login", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Processing Google login for email: ${email} (attempt ${attempts.count})`,
      );
      ...
      if (user) {
        ...
        if (displayName && user.displayName !== displayName) {
          console.log(
            `Updating displayName for user ${user.id} from "${user.displayName}" to "${displayName}"`,
          );
        ...
        console.log(
          `No user found with googleId ${googleId}, checking by email: ${email}`,
        );
        ...
        if (existingUserByEmail) {
          ...
          if (existingUserByEmail.authProvider === "local") {
            ...
            if (attempts.count >= MAX_ATTEMPTS) {
              console.log(
                `Blocking authentication for ${email} after ${attempts.count} attempts`,
              );
              ...
              return res.status(429).json({
                message:
                  "Too many authentication attempts. Authentication has been temporarily disabled for this account.",
                blockedUntil: attempts.blockUntil,
              });
            }
            ...
            return res.status(400).json({
              message:
                "An account with this email already exists. Please log in with your password first and then link your Google account.",
            });
          } else if (
            ...
            console.log(
              `Updating existing Google user with missing googleId: ${existingUserByEmail.id}`,
            );
            // In a full implementation, you would update the user here
        ...
        if (!user) {
          ...
          if (!attempts.signupRedirectSent) {
            ...
            console.log(
              `Blocking further authentication attempts for ${email} - signup redirect already sent`,
            );

            return res.status(429).json({
              message:
                "Signup redirect already sent. Please complete signup or wait before trying again.",
              requiresSignup: true,
      ...
      console.log(
        `Google authentication successful for user ID: ${user.id}, needs username: ${needsUsername}`,
      );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const passwordUpdated = await storage.updateUserPassword(
          resetRecord.userId,
          hashedPassword,
        );
        if (!passwordUpdated) {
  ...
  apiRouter.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const headerUserId =
          parseInt(req.headers["x-user-id"] as string, 10) || 0;
        const headerUserRole = (req.headers["x-user-role"] as string) || "";

        console.log(
          `Auth headers: User ID ${headerUserId}, Role: ${headerUserRole}`,
        );

        // Ensure the user is requesting their own reports or is an admin
        if (headerUserId !== userId && headerUserRole !== "admin") {
          console.log(
            `Access denied: headerUserId=${headerUserId}, userId=${userId}, role=${headerUserRole}`,
          );
          return res.status(403).json({
        ...
        console.error(
          `Error fetching scam reports for user ID ${req.params.userId}:`,
          error,
        );
        res
          .status(500)
  ...
  apiRouter.post("/scam-reports", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `🔑 User Authentication - ID: ${userId}, Email: ${userEmail}`,
      );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const userId = parseInt(
          (req.headers["x-user-id"] as string) || "0",
          10,
        );
        const userEmail = (req.headers["x-user-email"] as string) || "";
  ...
  apiRouter.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const user = (req as any).user;
        const userRole =
          user?.role || (req.headers["x-user-role"] as string) || "";
        ...
        if (req.query.isVerified !== undefined) {
          ...
          console.log(
            `Filtering scam reports by verification status: isVerified=${isVerified}`,
          );
          ...
          if (!isAdmin) {
            ...
            console.log(
              `Filtered reports for non-admin user, showing only published: ${reports.length}`,
            );
          }

          console.log(
            `Returning ${reports.length} ${isVerified ? "verified" : "unverified"} reports (page ${page})`,
          );
        } else {
          ...
          if (isAdmin) {
            ...
            console.log(
              `Admin user - returning paginated reports: ${reports.length} (page ${page})`,
            );
          } else {
            reports = await storage.getPublishedScamReports(page, limit);
            totalCount = await storage.getPublishedScamReportsCount();
            console.log(
              `Regular user - returning paginated published reports: ${reports.length} (page ${page})`,
            );
          }
        }
        ...
        console.log(
          `Returning ${reportsWithUsers.length} reports (page ${page} of ${Math.ceil(totalCount / limit)})`,
        );
  ...
  apiRouter.get("/scam-reports/recent", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `👤 User requesting recent reports - Role: ${userRole}, isAdmin: ${isAdmin}`,
      );
      ...
      const reportsWithUsers = await Promise.all(
        recentReports.map(async (report) => {
          ...
          const consolidation = await storage.getConsolidationForScamReport(
            report.id,
          );
          let consolidatedInfo = null;

          if (consolidation) {
            consolidatedInfo = await storage.getConsolidatedScam(
              consolidation.consolidatedScamId,
            );
          }
  ...
  apiRouter.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `👤 User requesting reports by type ${type} - Role: ${userRole}, isAdmin: ${isAdmin}`,
        );
        ...
        if (!isAdmin) {
          ...
          reports = reports.filter((report) => report.isPublished !== false);
          console.log(
            `After filtering: ${reports.length} published reports of type ${type}`,
          );
        }
  ...
  apiRouter.get("/scam-reports/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `👤 User requesting scam report ID ${id} - Role: ${userRole}, isAdmin: ${isAdmin}`,
      );
      ...
      if (!isAdmin && report.isPublished === false) {
        console.log(
          `⚠️ Non-admin user tried to access unpublished report ID ${id}`,
        );
        return res.status(403).json({
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (adminUser.role !== "admin") {
          console.error(
            `❌ Non-admin user ${adminUser.email} attempted to verify a report`,
          );
        ...
        if (originalReport.isVerified) {
          console.log(
            `ℹ️ Report ${id} is already verified by user ${originalReport.verifiedBy}`,
          );
          return res.json({
            success: true,
        ...
        // Verify the report
        console.log(
          `✅ Verifying scam report ID ${id} with admin ID ${adminUser.id}`,
        );
        ...
        console.log(
          `🎉 Successfully verified scam report: ${JSON.stringify(report)}`,
        );
        ...
        try {
          ...
          if (consolidation) {
            console.log(
              `🔄 Verifying associated consolidated scam ID ${consolidation.consolidatedScamId}`,
            );
            await storage.verifyConsolidatedScam(
              consolidation.consolidatedScamId,
            );
          }
        } catch (consolidationError) {
          console.error(
            "⚠️ Error verifying consolidated scam:",
            consolidationError,
          );
        ...
        console.error(
          `❌ Error verifying scam report ID ${req.params.id}:`,
          error,
        );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (adminUser.role !== "admin") {
          console.error(
            `❌ Non-admin user ${adminUser.email} attempted to publish a report`,
          );
          return res.status(403).json({ message: "Admin privileges required" });
        }
        ...
        console.error(
          `❌ Error publishing scam report ID ${req.params.id}:`,
          error,
        );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (adminUser.role !== "admin") {
          console.error(
            `❌ Non-admin user ${adminUser.email} attempted to unpublish a report`,
          );
          return res.status(403).json({ message: "Admin privileges required" });
        }
        ...
        console.error(
          `❌ Error unpublishing scam report ID ${req.params.id}:`,
          error,
        );
  ...
  apiRouter.get(
    "/consolidated-scams/by-type/:type",
    async (req: Request, res: Response) => {
      try {
        const type = req.params.type as "phone" | "email" | "business";
        ...
        console.error(
          `Error fetching ${req.params.type} consolidated scams:`,
          error,
        );
        res.status(500).json({ message: "Failed to fetch consolidated scams" });
      }
  ...
  apiRouter.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.error(
          `Error fetching consolidated scam ID ${req.params.id}:`,
          error,
        );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!adminUser || !adminUser.id) {
          console.error(
            "❌ Admin validation failed in verify consolidated scam endpoint",
          );
          return res.status(400).json({ message: "Valid admin user required" });
        }
        ...
        if (adminUser.role !== "admin") {
          console.error(
            `❌ Non-admin user ${adminUser.email} attempted to verify a consolidated scam`,
          );
          return res.status(403).json({ message: "Admin privileges required" });
        }
        ...
        console.log(
          `✓ Found consolidated scam: ${JSON.stringify(originalScam)}`,
        );
        ...
        if (!updatedScam) {
          console.error(
            `❌ Verification failed for consolidated scam ID ${id}`,
          );
          return res
            .status(500)
        ...
        console.log(
          `🎉 Successfully verified consolidated scam: ${JSON.stringify(updatedScam)}`,
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (typeof profileData.secondarySpecializations === "string") {
          profileData.secondarySpecializations = JSON.parse(
            profileData.secondarySpecializations,
          );
        }
        ...
        profileData.yearsOfExperience = parseInt(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (typeof updateData.secondarySpecializations === "string") {
          updateData.secondarySpecializations = JSON.parse(
            updateData.secondarySpecializations,
          );
        }
        ...
        if (updateData.yearsOfExperience !== undefined) {
          updateData.yearsOfExperience = parseInt(
        ...
        // Update the profile
        const updatedProfile = await storage.updateLawyerProfile(
          id,
          updateData,
        );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!admin || !admin.id) {
          console.error(
            "Admin validation failed in verify lawyer profile endpoint",
          );
          return res.status(400).json({ message: "Valid admin user required" });
        }
        ...
        console.log(
          `Verifying lawyer profile ID ${id} with admin ID ${admin.id}`,
        );
        ...
        console.log(
          `Successfully verified lawyer profile: ${JSON.stringify(verifiedProfile)}`,
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (request.lawyerProfileId) {
          const profile = await storage.getLawyerProfile(
        ...
        const updatedRequest = await storage.updateLawyerRequestStatus(
          id,
          status,
        );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedRequest = await storage.assignLawyerToRequest(
          id,
          profileId,
        );
  ...
  apiRouter.get("/consolidated-scams/:id/videos", async (req, res) => {
    try {
      ...
      const videos =
        await storage.getScamVideosForConsolidatedScam(consolidatedScamId);
      const transformed = videos.map(transformVideoForFrontend);
    ...
    try {
      ...
      if (!apiKey) {
        ...
        return res.status(200).json({
          response:
            "I'm sorry, but I'm currently operating in limited mode. My responses are based on pre-defined information about common scams. For more detailed assistance, please try again later when my full capabilities are available.",
          source: "fallback",
        });
      }
      ...
      const formattedMessages = [
        {
          role: "system",
          content:
            "You are a helpful assistant for scam victims. Provide specific, actionable advice for people who've been scammed. Be supportive and empathetic, but focus on practical steps they can take. Include references to official resources when appropriate. Keep your responses concise and direct.",
        },
...
<p>${message.replace(/\n/g, "<br>")}</p>
  ...
  apiRouter.post(
    "/security-checklist/:itemId/progress",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const user = (req as any).user;
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Admin ${user.id} creating security checklist item:`,
          itemData,
        );
  ...
  apiRouter.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Admin ${user.id} updating security checklist item ${itemId}:`,
          updates,
        );

        const updatedItem = await storage.updateSecurityChecklistItem(
          itemId,
          updates,
        );
        if (!updatedItem) {
  ...
  apiRouter.delete(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Admin ${user.id} deleting security checklist item ${itemId}`,
        );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `📡 Using API config: ${apiConfig.name} for ${type} lookup`,
        );

        // Perform scam lookup
        const scamLookupService = ScamLookupService.getInstance();
        const result = await scamLookupService.lookupScamData(
          type,
          input,
          apiConfig,
        );
        ...
        console.log(
          `✅ Scam lookup result: ${result.status} (Risk: ${result.riskScore})`,
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const configs = allConfigs.filter(
          (config) => config.enabled && config.type === type,
        );
        ...
        for (const config of configs) {
          ...
          try {
            ...
            const { ScamLookupService } = await import(
              "./scamLookupService.js"
            );
            const lookupService = ScamLookupService.getInstance();
            const result = await lookupService.lookupScamData(
              type,
              value,
              config,
            );
            ...
            const responseTime = Date.now() - startTime;
            console.error(
              `Lookup error for ${config.name} (${type}:${value}):`,
              lookupError,
            );
  ...
  apiRouter.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Returning ${publicConfigs.length} public API configurations`,
        );
  ...
  apiRouter.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `🧪 API Test Request - ID: ${id}, Type: ${type}, Input: ${testInput}`,
        );
        ...
        // Test the API
        const scamLookupService = ScamLookupService.getInstance();
        const testResult = await scamLookupService.testApiConfig(
          type,
          config,
          testInput,
        );
