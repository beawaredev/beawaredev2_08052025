export class ScamLookupService {
    ...
    async lookupScamData(type, input, apiConfig) {
    ...
    parseIPQSResponse(type, input, data, provider, apiCallDetails) {
        let riskScore = 0;
        ...
        switch (type) {
            case 'phone':
                ...
                if (data.VOIP)
                    details.riskFactors.push('VOIP');
                if (data.prepaid)
                    details.riskFactors.push('Prepaid');
                if (data.risky)
                    details.riskFactors.push('Risky');
                break;
            case 'email':
                riskScore = data.fraud_score || 0;
                details = {
                ...
                if (data.disposable)
                    details.riskFactors.push('Disposable');
                if (data.suspect)
                    details.riskFactors.push('Suspicious');
                if (data.recent_abuse)
                    details.riskFactors.push('Recent Abuse');
                break;
            case 'url':
                ...
                if (data.malware)
                    details.riskFactors.push('Malware');
                if (data.phishing)
                    details.riskFactors.push('Phishing');
                if (data.suspicious)
                    details.riskFactors.push('Suspicious');
                break;
            case 'ip':
                ...
                if (data.vpn)
                    details.riskFactors.push('VPN');
                if (data.tor)
                    details.riskFactors.push('Tor');
                if (data.proxy)
                    details.riskFactors.push('Proxy');
                if (data.bot_status)
                    details.riskFactors.push('Bot');
                break;
        }
    ...
    async lookupWithGenericAPI(type, input, apiConfig) {
        try {
            ...
            if (body)
                console.log(`📦 Body:`, processedParams);
    ...
    createErrorResult(type, input, provider, error, apiCallDetails) {
        return {
            type,
            input,
            provider,
    ...
    createNotImplementedResult(type, input, provider) {
        return {
            type,
            input,
            provider,
    ...
    async testApiConfig(type, apiConfig, customInput) {
        const testInputs = {
            phone: '+1234567890',
