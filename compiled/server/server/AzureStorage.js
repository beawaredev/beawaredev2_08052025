...
class AzureStorage {
    async ensureConnection() {
        try {
            if (db_js_1.pool.connected) {
                return;
            }
            if (db_js_1.pool.connecting) {
                // Wait for existing connection attempt
                await new Promise((resolve) => {
                    const checkInterval = setInterval(() => {
                        if (db_js_1.pool.connected || !db_js_1.pool.connecting) {
            ...
            try {
                if (db_js_1.pool && !db_js_1.pool.connected) {
                    await db_js_1.pool.close();
                }
            }
            catch (closeError) {
    ...
    async getUser(id) {
        try {
            ...
            const request = db_js_1.pool.request();
    ...
    async updateUserSecurityProgress(userId, checklistItemId, isCompleted, notes) {
      ...
      `);
            ...
            if (existingResult.recordset.length > 0) {
                ...
                result = await db_js_1.pool.request().query(`
    ...
    async createPasswordReset(userId, resetToken) {
        ...
        try {
            ...
            const result = await db_js_1.pool.request()
                .input('userId', mssql_1.default.Int, userId)
    ...
    async updateUserPassword(userId, newPassword) {
        ...
        try {
            const result = await db_js_1.pool.request()
                .input('userId', mssql_1.default.Int, userId)
                .input('password', mssql_1.default.NVarChar, newPassword)
