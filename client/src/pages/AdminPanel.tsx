    ...
    /** ----------------------------------------------------------------------
     * Admin Panel
     * Tabs: Videos (with Live Preview), Security Checklist, Scam APIs
     * This version FIXES the "Unsupported or invalid video source" error in the
     * Admin Videos tab by rendering a Live Preview that uses the same player logic
     * as the public ScamVideos page (ScamVideoPlayer).
     * ---------------------------------------------------------------------- */
    export default function AdminPanel() {
      ...
      const { data: videosRaw, isLoading: isVideosLoading } = useQuery<any[]>({
        queryKey: ["/api/scam-videos", "admin-preview"],
        queryFn: async () => {
          const res = await fetch(`${API}/api/scam-videos`, { credentials: "include" });
      ...
      const videos: VideoItem[] = useMemo(
        () => (videosRaw || []).map(normalizeVideo).filter(v => !!v.videoUrl?.trim()),
        [videosRaw]
      );

      /** ===================== API Configs (unchanged) ===================== */
      const {
        ...
        queryFn: async () => {
          ...
          if (!res.ok) throw new Error(`Failed to fetch API configs: ${res.status} ${res.statusText}`);
          return res.json();
        },
      ...
      const apiCreateMutation = useMutation({
        ...
        onSuccess: () => {
          ...
          toast({ title: "API Configuration Created", description: "The API configuration has been created successfully." });
        },
        onError: (error: any) => {
          toast({ title: "Creation Failed", description: `Failed to create API configuration: ${error?.message || error}`, variant: "destructive" });
        },
      });
      ...
      const apiUpdateMutation = useMutation({
        ...
        onSuccess: () => {
          ...
          apiEditForm.reset();
          globalQueryClient.invalidateQueries({ queryKey: ["/api/api-configs"] });
          toast({ title: "API Configuration Updated", description: "The API configuration has been updated successfully." });
        },
        onError: (error: any) => {
          toast({ title: "Update Failed", description: `Failed to update API configuration: ${error?.message || error}`, variant: "destructive" });
        },
      });

      const apiDeleteMutation = useMutation({
        mutationFn: async (id: number) => {
          const res = await apiRequest(`/api/api-configs/${id}`, { method: "DELETE" });
          if (!res.ok) throw new Error(`Delete failed: ${res.status} ${res.statusText}`);
          return res.json?.() ?? res;
        },
        onSuccess: () => {
          globalQueryClient.invalidateQueries({ queryKey: ["/api/api-configs"] });
          toast({ title: "API Configuration Deleted", description: "The API configuration has been deleted successfully." });
        },
        onError: (error: any) => {
          toast({ title: "Deletion Failed", description: `Failed to delete API configuration: ${error?.message || error}`, variant: "destructive" });
        },
      });
      ...
      const handleTestLookup = async (type: string, input: string) => {
        ...
        try {
          ...
          toast({ title: "Test Completed", description: `Scam lookup test completed for ${type}: ${input}` });
        } catch (error: any) {
          toast({ title: "Test Failed", description: `Scam lookup test failed: ${error?.message || error}`, variant: "destructive" });
        }
      };
      ...
      const handleTestApi = async (config: any, customInput?: string) => {
        ...
        try {
          toast({ title: "Testing API", description: `Testing ${config.name} with input: ${testInput}` });
          ...
          toast({
            ...
            description: result?.success ? `${config.name} is working correctly.` : result?.message || "Unknown error",
            variant: result?.success ? "default" : "destructive",
          });
        } catch (error: any) {
          setApiTestResults((prev) => ({
            ...prev,
            [config.id]: { success: false, error: error?.message || String(error), timestamp: new Date().toISOString(), testInput },
          }));
          toast({ title: "API Test Error", description: `Test failed for ${config.name}: ${error?.message || error}`, variant: "destructive" });
        } finally {
          setTestingApiIds((prev) => { const s = new Set(prev); s.delete(config.id); return s; });
        }
      };
      ...
      const onSubmitApiEdit = (data: any) => {
        if (!data.name || !data.type || !data.url || !data.apiKey) {
          toast({ title: "Validation Error", description: "Please fill in all required fields (Name, Type, URL, API Key)", variant: "destructive" });
          return;
        }
        ...
        <div className="space-y-6">
          ...
          <Tabs defaultValue="security" onValueChange={setActiveTab}>
            ...
            <TabsContent value="videos" className="mt-6">
              {/* Keep your existing manager for CRUD */}
              <ScamVideoManager />

              {/* Live Preview — uses ScamVideoPlayer like public page */}
              <Card className="mt-6">
                <CardHeader className="pb-2">
                  <CardTitle>Live Preview</CardTitle>
                  <CardDescription>
                    This preview uses the same player as the public Videos page, so you can verify URLs render correctly.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {isVideosLoading ? (
                    ...
                    <div className="col-span-full text-center py-8">
                      ...
                      <p className="text-muted-foreground">No videos to preview yet.</p>
                    </div>
                  ) : (
                    videos.map((v) => {
                      const ytId = extractYouTubeId(v.videoUrl);
                      const thumb =
                        v.thumbnailUrl || (ytId ? `https://img.youtube.com/vi/${ytId}/hqdefault.jpg` : null);
                      return (
                        <Card key={v.id} className="overflow-hidden hover:shadow-md transition-shadow">
                          <CardHeader className="pb-2">
                            <CardTitle className="text-base">{v.title}</CardTitle>
                            <CardDescription>
                              {v.createdAt ? `Added ${new Date(v.createdAt).toLocaleDateString()}` : "Recently added"}
                            </CardDescription>
                          </CardHeader>
                          <CardContent>
            ...
            <TabsContent value="api-configs" className="mt-6">
              <div className="space-y-6">
                ...
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="text-xl font-semibold tracking-tight">Scam Detection API Configuration</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <div className="flex gap-2">
                          <Input id="test-phone" value={testPhone} onChange={(e) => setTestPhone(e.target.value)} placeholder="+1234567890" />
                          <Button onClick={() => handleTestLookup("phone", testPhone)} size="sm">Test</Button>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button onClick={() => handleTestLookup("email", testEmail)} size="sm">Test</Button>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button onClick={() => handleTestLookup("url", testUrl)} size="sm">Test</Button>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                          ...
                          <div key={config.id} className="border rounded-lg">
                            <div className="flex items-center justify-between p-4">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleTestApi(config)} disabled={testingApiIds.has(config.id)}>
                                  {testingApiIds.has(config.id) ? "Testing..." : "Test"}
                                </Button>
                                <Button variant="outline" size="sm" onClick={() => handleEditApiConfig(config)}>Edit</Button>
                                <Button variant="outline" size="sm" onClick={() => apiDeleteMutation.mutate(config.id)}>
                                  <X className="h-4 w-4" />
                                </Button>
                              </div>
                            </div>
                              ...
                              <div className="px-4 py-3 border-t bg-blue-50 dark:bg-blue-900/20">
                                <div className="space-y-3">
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    />
                                    <Button size="sm" onClick={() => handleTestApi(config, customTestInputs[config.id])} disabled={testingApiIds.has(config.id)}>
                              ...
                              <div className="px-4 pb-4 border-t bg-slate-50 dark:bg-slate-900/50">
                                <div className="mt-3">
                                  <div className="flex items-center justify-between mb-2">
                                    <h5 className="text-sm font-medium text-slate-700 dark:text-slate-300">Test Results</h5>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {apiTestResults[config.id].apiCallDetails && (
                                      <div className="mt-3 p-3 bg-slate-100 dark:bg-slate-800 rounded">
                                        <h6 className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">API Call Details</h6>
                                        <div className="space-y-1">
                                          <div className="flex items-start gap-2">
                                            <span className="text-xs font-medium min-w-[50px]">Method:</span>
                                            <code className="text-xs bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded">
                                              {apiTestResults[config.id].apiCallDetails.method}
                                            </code>
                                          </div>
                                          <div className="flex items-start gap-2">
                                            <span className="text-xs font-medium min-w-[50px]">URL:</span>
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Badge variant={apiTestResults[config.id].success ? "default" : "destructive"} className="text-xs">
                                    ...
                                    {apiTestResults[config.id].testResult && (
                                      <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-2">
                                          <span className="text-xs font-medium">Status:</span>
                                          <Badge variant="outline" className="text-xs">{apiTestResults[config.id].testResult.status}</Badge>
                                        </div>
                                        <div className="flex items-center gap-2">
                                    ...
                                    {apiTestResults[config.id].testResult?.details && (
                                      <details className="mt-2">
                                        <summary className="text-xs font-medium cursor-pointer">Raw Response</summary>
                      ...
                      {apiConfigsError && (
                        <div className="text-center py-8 text-red-600">
                          ...
                          <p className="text-sm mt-2">{(apiConfigsError as Error).message}</p>
                        </div>
                      )}
                    </div>
                  </CardContent>
                    ...
                    <form onSubmit={apiCreateForm.handleSubmit(onSubmitApiCreate)} className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input id="api-name" {...apiCreateForm.register("name", { required: "Name is required" })} placeholder="IPQualityScore Phone" />
                          {apiCreateForm.formState.errors.name && (
                            <p className="text-xs text-red-500 mt-1">{apiCreateForm.formState.errors.name.message as string}</p>
                          )}
                        </div>
                        <div>
                          ...
                          {apiCreateForm.formState.errors.type && (
                            <p className="text-xs text-red-500 mt-1">{apiCreateForm.formState.errors.type?.message as string}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <Input id="api-url" {...apiCreateForm.register("url", { required: "URL is required" })} placeholder="https://api.example.com/v1/check" />
                        {apiCreateForm.formState.errors.url && (
                          <p className="text-xs text-red-500 mt-1">{apiCreateForm.formState.errors.url.message as string}</p>
                        )}
                      </div>

                      <div>
                        <Label htmlFor="api-key">API Key *</Label>
                        <Input id="api-key" type="password" {...apiCreateForm.register("apiKey", { required: "API Key is required" })} placeholder="your-api-key-here" />
                        {apiCreateForm.formState.errors.apiKey && (
                          <p className="text-xs text-red-500 mt-1">{apiCreateForm.formState.errors.apiKey.message as string}</p>
                        )}
                      </div>

                      <div>
                        <Label htmlFor="api-description">Description</Label>
                        <Textarea id="api-description" {...apiCreateForm.register("description")} placeholder="Brief description of this API service" />
                      </div>

                      <div>
                        <Label htmlFor="api-params">Parameter Mapping (JSON)</Label>
                        <Textarea id="api-params" {...apiCreateForm.register("parameterMapping")} rows={4} className="font-mono text-sm" />
                        <p className="text-xs text-muted-foreground mt-1">Use {"{{input}}"} and {"{{apiKey}}"} — replaced at runtime.</p>
                      </div>

                      <div>
                        <Label htmlFor="api-headers">Headers (JSON)</Label>
                        <Textarea id="api-headers" {...apiCreateForm.register("headers")} rows={3} className="font-mono text-sm" />
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="api-rate-limit">Rate Limit (per minute)</Label>
                          <Input id="api-rate-limit" type="number" {...apiCreateForm.register("rateLimit", { valueAsNumber: true })} placeholder="100" />
                        </div>
                        <div>
                          <Label htmlFor="api-timeout">Timeout (seconds)</Label>
                          <Input id="api-timeout" type="number" {...apiCreateForm.register("timeout", { valueAsNumber: true })} placeholder="30" />
                        </div>
                      </div>

                      <div className="flex justify-end gap-2">
                        <Button type="button" variant="outline" onClick={() => setIsApiCreateDialogOpen(false)}>Cancel</Button>
                    ...
                    <form onSubmit={apiEditForm.handleSubmit(onSubmitApiEdit)} className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input id="edit-api-name" {...apiEditForm.register("name", { required: "Name is required" })} />
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <Input id="edit-api-url" {...apiEditForm.register("url", { required: "URL is required" })} />
                      </div>
                      <div>
                        <Label htmlFor="edit-api-key">API Key *</Label>
                        <Input id="edit-api-key" type="password" {...apiEditForm.register("apiKey", { required: "API Key is required" })} />
                      </div>
                      <div>
                        <Label htmlFor="edit-api-description">Description</Label>
                        <Textarea id="edit-api-description" {...apiEditForm.register("description")} />
                      </div>
                      <div>
                        <Label htmlFor="edit-api-params">Parameter Mapping (JSON)</Label>
                        <Textarea id="edit-api-params" {...apiEditForm.register("parameterMapping")} rows={4} className="font-mono text-sm" />
                      </div>
                      <div>
                        <Label htmlFor="edit-api-headers">Headers (JSON)</Label>
                        <Textarea id="edit-api-headers" {...apiEditForm.register("headers")} rows={3} className="font-mono text-sm" />
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          <Label htmlFor="edit-api-rate-limit">Rate Limit</Label>
                          <Input id="edit-api-rate-limit" type="number" {...apiEditForm.register("rateLimit", { valueAsNumber: true })} />
                        </div>
                        <div>
                          <Label htmlFor="edit-api-timeout">Timeout</Label>
                          <Input id="edit-api-timeout" type="number" {...apiEditForm.register("timeout", { valueAsNumber: true })} />
                        </div>
                        <div className="flex items-center space-x-2">
                          <input id="edit-api-enabled" type="checkbox" {...apiEditForm.register("enabled")} className="rounded" />
                          <Label htmlFor="edit-api-enabled">Enabled</Label>
                        </div>
                      </div>

                      <div className="flex justify-end gap-2">
                        <Button type="button" variant="outline" onClick={() => setIsApiEditDialogOpen(false)}>Cancel</Button>
    ...
    /** ==================================================================
     * Admin Security Checklist Panel (unchanged from your last version)
     * ================================================================== */
    function AdminSecurityChecklistPanel() {
      ...
      const { data: checklistItems = [], isLoading } = useQuery<any[]>({
        ...
        queryFn: async () => {
          ...
          if (!res.ok) throw new Error(`Failed to fetch checklist: ${res.status} ${res.statusText}`);
          return res.json();
        },
      });
      ...
      const updateItemMutation = useMutation({
        mutationFn: async (data: any) => {
          const res = await apiRequest(`/api/security-checklist/${editingItem.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(data),
          });
        ...
        onSuccess: () => {
          ...
          setEditingItem(null);
          toast({ title: "Component Updated", description: "Security checklist component has been updated successfully." });
        },
        onError: (error: any) => {
          toast({ title: "Update Failed", description: `Failed to update component: ${error?.message || error}`, variant: "destructive" });
        },
      });
      ...
      const createItemMutation = useMutation({
        ...
        onSuccess: () => {
          ...
          createForm.reset();
          toast({ title: "Component Created", description: "New security checklist component has been created successfully." });
        },
        onError: (error: any) => {
          toast({ title: "Creation Failed", description: `Failed to create component: ${error?.message || error}`, variant: "destructive" });
        },
      });

      const deleteItemMutation = useMutation({
        mutationFn: async (itemId: number) => {
          const res = await apiRequest(`/api/security-checklist/${itemId}`, { method: "DELETE" });
          if (!res.ok) throw new Error(`Delete failed: ${res.status} ${res.statusText}`);
          return res.json?.() ?? res;
        },
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ["/api/security-checklist"] });
          toast({ title: "Component Deleted", description: "Security checklist component has been deleted successfully." });
        },
        onError: (error: any) => {
          toast({ title: "Deletion Failed", description: `Failed to delete component: ${error?.message || error}`, variant: "destructive" });
        },
      });

      const onSubmitEdit = (values: any) => updateItemMutation.mutate(values);
      const onSubmitCreate = (values: any) => {
        if (!values.title?.trim() || !values.description?.trim()) {
          toast({ title: "Validation Error", description: "Please provide both Title and Description.", variant: "destructive" });
          return;
        }
      ...
      if (isLoading) return <div className="text-center py-8">Loading security checklist...</div>;
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold tracking-tight">Security Checklist Management</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card><CardContent className="p-4"><div className="text-2xl font-bold text-blue-600">{checklistItems.length}</div><div className="text-xs text-muted-foreground">Total Components</div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="text-2xl font-bold text-green-600">{checklistItems.filter((i: any) => i.priority === "high").length}</div><div className="text-xs text-muted-foreground">High Priority</div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="text-2xl font-bold text-purple-600">{checklistItems.filter((i: any) => i.toolLaunchUrl).length}</div><div className="text-xs text-muted-foreground">With Tools</div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="text-2xl font-bold text-orange-600">{checklistItems.filter((i: any) => i.youtubeVideoUrl).length}</div><div className="text-xs text-muted-foreground">With Videos</div></CardContent></Card>
          </div>

          {/* Category Filter */}
          <div className="flex gap-2 flex-wrap">
            {Array.from(new Set(["all", ...checklistItems.map((i: any) => i.category)])).map((category) => (
            ...
            {filteredItems.map((item: any) => (
              <Card key={item.id} className="relative">
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleEditItem(item)} className="h-6 w-6 p-0">
                        <Edit className="h-3 w-3" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => handleDeleteItem(item.id)} className="h-6 w-6 p-0 text-red-600 hover:text-red-800">
                ...
                <CardContent className="pt-2">
                  ...
                  <div className="text-xs text-muted-foreground"><span className="font-medium">Time:</span> {item.estimatedTimeMinutes} min</div>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle>Add New Security Component</DialogTitle></DialogHeader>
              <form onSubmit={createForm.handleSubmit(onSubmitCreate)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="create-title">Title</Label>
                    <Input id="create-title" {...createForm.register("title", { required: true })} placeholder="Component title" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <Textarea id="create-description" {...createForm.register("description", { required: true })} placeholder="Brief description" />
                </div>
                <div>
                  <Label htmlFor="create-recommendation">Recommendation Text</Label>
                  <Textarea id="create-recommendation" {...createForm.register("recommendationText")} placeholder="Detailed recommendation" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Input id="create-time" type="number" {...createForm.register("estimatedTimeMinutes", { valueAsNumber: true })} placeholder="15" />
                  </div>
                </div>
                <div>
                  <Label htmlFor="create-help-url">Help URL</Label>
                  <Input id="create-help-url" {...createForm.register("helpUrl")} placeholder="https://example.com/help" />
                </div>
                <div>
                  <Label htmlFor="create-tool-url">Tool Launch URL (optional)</Label>
                  <Input id="create-tool-url" {...createForm.register("toolLaunchUrl")} placeholder="https://tool.example.com" />
                </div>
                <div>
                  <Label htmlFor="create-video-url">YouTube Video URL (optional)</Label>
                  <Input id="create-video-url" {...createForm.register("youtubeVideoUrl")} placeholder="https://youtube.com/watch?v=..." />
                </div>
                <div className="flex justify-end gap-2">
          ...
          <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle>Edit Security Component</DialogTitle></DialogHeader>
              <form onSubmit={editForm.handleSubmit(onSubmitEdit)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="edit-title">Title</Label>
                    <Input id="edit-title" {...editForm.register("title", { required: true })} placeholder="Component title" />
                  </div>
                  <div>
                    <Label htmlFor="edit-category">Category</Label>
                    <Select value={editForm.watch("category")} onValueChange={(v) => editForm.setValue("category", v)}>
                      <SelectTrigger><SelectValue placeholder="Select category" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="account_security">Account Security</SelectItem>
                        <SelectItem value="password_security">Password Security</SelectItem>
                ...
                <div>
                  ...
                  <Textarea id="edit-description" {...editForm.register("description", { required: true })} placeholder="Brief description" />
                </div>
                <div>
                  <Label htmlFor="edit-recommendation">Recommendation Text</Label>
                  <Textarea id="edit-recommendation" {...editForm.register("recommendationText")} placeholder="Detailed recommendation" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Input id="edit-time" type="number" {...editForm.register("estimatedTimeMinutes", { valueAsNumber: true })} placeholder="15" />
                  </div>
                </div>
                <div>
                  <Label htmlFor="edit-help-url">Help URL</Label>
                  <Input id="edit-help-url" {...editForm.register("helpUrl")} placeholder="https://example.com/help" />
                </div>
                <div>
                  <Label htmlFor="edit-tool-url">Tool Launch URL (optional)</Label>
                  <Input id="edit-tool-url" {...editForm.register("toolLaunchUrl")} placeholder="https://tool.example.com" />
                </div>
                <div>
                  <Label htmlFor="edit-video-url">YouTube Video URL (optional)</Label>
                  <Input id="edit-video-url" {...editForm.register("youtubeVideoUrl")} placeholder="https://youtube.com/watch?v=..." />
                </div>
                <div className="flex justify-end gap-2">
